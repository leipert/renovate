...
async function getPrList(branchPrefix) {
  if (!config.prList) {
    if (branchPrefix) {
      const branches = await getAllRenovateBranches(branchPrefix);

      const PRs = await Promise.all(branches.map(branch => findPr(branch)));

      config.prList = PRs.map(mapPullRequests);
    } else {
      const urlString = `projects/${
        config.repository
